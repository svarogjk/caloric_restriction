...
const AnalysisResultsDisplay: React.FC<AnalysisResultsDisplayProps> = ({ results, onClose }) => {
    ...
    // The exported figure is for papers and slides, so it is captured LIGHT
    // regardless of the app theme — a dark-background plot is unusable in a
    // manuscript. data-theme re-points the token layer for this subtree only,
    // and is restored in `finally` so an export failure can't strand the UI.
    const handleExportPNG = async () => {
        const node = volcanoRef.current
        if (!node) return
        const previousTheme = node.getAttribute('data-theme')
        ...
        try {
            ...
            if (previousTheme === null) node.removeAttribute('data-theme')
            else node.setAttribute('data-theme', previousTheme)
        }
    }
    ...
    const CustomTooltip: React.FC<{ active?: boolean; payload?: Array<{ payload: TransformedGene }> }> = ({ active, payload }) => {
        if (active && payload && payload[0]) {
            ...
            return (
                <div className="bg-surface p-3 border border-border-strong rounded shadow-lg z-50">
                    ...
                    <div className="mt-2 space-y-1">
                        <p className="text-sm text-fg-muted">
                            ...
                            <span className={data.hazard_ratio > 1 ? 'text-danger' : 'text-accent-fg'}>
    ...
    return (
        <div className="bg-surface rounded-lg border border-border shadow-sm overflow-hidden">
            ...
            {/* Header */}
            <div className="bg-gradient-to-r from-accent to-alt px-4 py-3 flex items-center justify-between">
                <div>
                    <h3 className="text-on-accent font-semibold">Survival Analysis Results</h3>
                    <p className="text-on-accent text-sm">Query: {results.query}</p>
                </div>
                <div className="flex items-center gap-2">
                    {/* Auto-save toggle */}
                    <label className="flex items-center gap-1 cursor-pointer" title="Auto-save results to database">
                        <span className="text-xs text-on-accent">Auto-save</span>
                        <div
                            onClick={() => dispatch(setAutoSave(!autoSave))}
                            className={`relative w-8 h-4 rounded-full transition-colors cursor-pointer ${autoSave ? 'bg-ok' : 'bg-white/30'}`}
                        >
                            <div className={`absolute top-0.5 w-3 h-3 bg-surface rounded-full shadow transition-transform ${autoSave ? 'translate-x-4' : 'translate-x-0.5'}`} />
                        </div>
                    </label>
                    ...
                    {!results.result_id && (
                        <button
                            onClick={handleSave}
                            disabled={isSaving}
                            className="text-xs px-2 py-1 rounded border transition-colors bg-white/20 text-on-accent border-white/30 hover:bg-white/30 disabled:opacity-60 disabled:cursor-not-allowed"
                    ...
                    {results.result_id && (
                        <button
                            ...
                            className={`text-xs px-2 py-1 rounded border transition-colors ${
                                shareCopied
                                    ? 'bg-ok text-on-accent border-ok-border'
                                    : 'bg-white/20 text-on-accent border-white/30 hover:bg-white/30'
                            }`}
                    ...
                    <button
                        ...
                        className="text-xs px-2 py-1 rounded border transition-colors bg-white/20 text-on-accent border-white/30 hover:bg-white/30"
                    ...
                    {onClose && (
                        <button
                            onClick={onClose}
                            className="text-on-accent hover:bg-white/20 rounded p-1"
                        >
            ...
            {/* Stats Summary */}
            <div className="grid grid-cols-3 gap-2 p-3 bg-surface-sunken border-b">
                <div className="text-center">
                    <p className="text-2xl font-bold text-accent-fg">{results.n_datasets_analyzed}</p>
                    <p className="text-xs text-fg-muted">Datasets Analyzed</p>
                </div>
                <div className="text-center">
                    <p className="text-2xl font-bold text-ok">{results.n_datasets_with_survival}</p>
                    <p className="text-xs text-fg-muted">With Survival Data</p>
                </div>
                <div className="text-center">
                    <p className="text-2xl font-bold text-alt">{results.common_genes.length}</p>
                    <p className="text-xs text-fg-muted">Survival Genes</p>
                </div>
            </div>
                ...
                {activeTab === 'summary' && (
                    <div className="space-y-4">
                        ...
                        {results.ranking_recommendations && (
                            <div className="p-3 bg-accent-soft rounded-lg border border-border-accent">
                                <div className="flex items-start gap-2">
                                    <svg className="w-5 h-5 text-accent-fg mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-accent-fg">{results.ranking_recommendations}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {sortedGenes.length > 0 && (
                            <div>
                                <h4 className="font-medium text-fg-strong mb-2 flex items-center gap-1">
                                ...
                                <div className="space-y-2">
                                    {sortedGenes.slice(0, 5).map((gene) => (
                                        <div
                                            key={gene.gene_id}
                                            className="flex items-center justify-between p-2 bg-surface-sunken rounded cursor-pointer hover:bg-surface-sunken"
                                            onClick={() => setSelectedGene(gene)}
                                        >
                                            <div>
                                                <span className="font-medium">{gene.gene_symbol || gene.gene_id}</span>
                                                <span className="text-xs text-fg-muted ml-2">({gene.n_datasets} datasets)</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className={`text-sm font-medium ${
                                                    gene.predominant_risk === 'high_risk' ? 'text-danger' : 'text-accent-fg'
                                                }`}>
                        ...
                        <div className="mt-4">
                            ...
                            {paramsOpen && (
                                <div className="mt-2 bg-surface-sunken border border-border rounded p-3 text-xs text-fg-muted">
                    ...
                    <div>
                        ...
                        <div className="flex justify-end mb-2">
                            <button
                                ...
                                className="px-3 py-1.5 text-xs bg-surface-sunken text-fg rounded hover:bg-surface-hover border border-border-strong"
                            >
                                Export PNG
                            </button>
                        </div>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <ScatterChart margin={{ top: 10, right: 10, bottom: 30, left: 40 }}>
                                    ...
                                    <YAxis
                                        ...
                                        fontSize={12}
                                    />
                                    <ReferenceLine x={0} stroke="var(--color-chart-axis)" strokeDasharray="3 3" />
                                    <ReferenceLine y={-Math.log10(0.05)} stroke="var(--color-chart-axis)" strokeDasharray="3 3" />
                        ...
                        <div className="flex gap-4 justify-center mt-2 text-xs">
                            ...
                            <div className="flex items-center gap-1">
                                <div className="w-2 h-2 bg-fg-faint rounded-full"></div>
                                <span>NS</span>
                            </div>
                        </div>
                        <p className="text-xs text-fg-faint text-center mt-1">Click a gene to view Kaplan-Meier curves</p>
                    </div>
                )}
                ...
                {activeTab === 'genes' && (
                    <div>
                        ...
                        <div className="flex items-center justify-between mb-3 gap-2">
                            <span className="text-sm text-fg-muted font-medium whitespace-nowrap">
                            ...
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="px-2 py-1 text-sm border border-border-strong rounded focus:outline-none focus:ring-1 focus:ring-accent-ring w-36"
                                />
                                <button
                                    onClick={handleExportCSV}
                                    className="px-3 py-1 text-xs bg-surface-sunken text-fg rounded hover:bg-surface-hover border border-border-strong whitespace-nowrap"
                                >
                                    Export CSV
                                </button>
                            </div>
                        ...
                        <div className="space-y-2 max-h-96 overflow-y-auto">
                            {pagedGenes.map((gene) => (
                                ...
                                >
                                    <div
                                        className="flex items-center justify-between p-3 bg-surface-sunken cursor-pointer hover:bg-surface-sunken"
                                        onClick={() => setExpandedGeneId(expandedGeneId === gene.gene_id ? null : gene.gene_id)}
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className={`w-3 h-3 rounded-full flex-shrink-0 ${
                                                gene.predominant_risk === 'high_risk' ? 'bg-danger' : 'bg-accent'
                                            }`}></span>
                                            <div>
                                                <span className="font-medium">{gene.gene_symbol || gene.gene_id}</span>
                                                {gene.gene_symbol && (
                                                    <span className="text-xs text-fg-muted ml-1">({gene.gene_id})</span>
                                                )}
                                                {gene.is_predictive && (
                                                    <span
                                                        className="ml-2 px-1.5 py-0.5 text-[10px] font-semibold rounded bg-warn-soft text-warn align-middle"
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="text-right text-sm">
                                                <div className={gene.avg_hazard_ratio > 1 ? 'text-danger' : 'text-accent-fg'}>
                                                    HR: {gene.avg_hazard_ratio.toFixed(2)}
                                                </div>
                                            ...
                                            <svg
                                                className={`w-5 h-5 text-fg-faint transition-transform ${
                                    ...
                                    {expandedGeneId === gene.gene_id && (
                                        <div className="p-3 border-t bg-surface">
                                            <div className="grid grid-cols-2 gap-2 text-sm">
                                                <div>
                                                    <span className="text-fg-muted">Datasets:</span>
                                                    <span className="ml-1 font-medium">{gene.n_datasets}</span>
                                                </div>
                                                <div>
                                                    <span className="text-fg-muted">Consistency:</span>
                                                ...
                                                return (
                                                    <div className="mt-3 pt-3 border-t border-dashed border-border">
                                                        <div className="flex items-center gap-1 mb-1">
                                                            ...
                                                            <span
                                                                className="text-[10px] text-fg-faint cursor-help"
                                                        ...
                                                        <div className="overflow-x-auto">
                                                            <table className="w-full text-xs">
                                                                ...
                                                                <tbody>
                                                                    {adjusted.map((d) => (
                                                                        <tr key={d.dataset_id} className="text-fg-muted">
                                                                            ...
                                                                            <td className={`pr-2 py-0.5 font-medium ${
                                                                                (d.adjusted_hazard_ratio ?? 1) > 1 ? 'text-danger' : 'text-accent-fg'
                                                                            }`}>
                                            ...
                                            <button
                                                onClick={() => setSelectedGene(gene)}
                                                className="mt-2 w-full py-1.5 text-sm bg-accent-soft text-accent-fg rounded hover:bg-accent-soft"
                                            >
                            ...
                            {totalFilteredGenes === 0 && (
                                <p className="text-sm text-fg-faint text-center py-4">No genes match your search.</p>
                            )}
                        </div>
                            ...
                            <div className="flex items-center justify-between mt-3 pt-3 border-t">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        disabled={genePage === 0}
                                        className="px-3 py-1 text-xs bg-surface-sunken text-fg rounded hover:bg-surface-hover disabled:opacity-40 disabled:cursor-not-allowed border border-border-strong"
                                    >
                                        Previous
                                    </button>
                                    <button
                ...
                {activeTab === 'forest' && (
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            <label className="text-xs text-fg-muted">Gene:</label>
                            <select
                                value={selectedForestGeneId ?? selectedForestGene?.gene_id ?? ''}
                                onChange={(e) => setSelectedForestGeneId(e.target.value)}
                                className="text-sm border border-border-strong rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-accent-ring"
                            >
                ...
                {activeTab === 'methods' && (
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <h4 className="text-sm font-medium text-fg">Auto-Generated Methods Text</h4>
                            <button
                        ...
                        <textarea
                            ...
                            value={methodsText}
                            rows={8}
                            className="w-full text-sm text-fg bg-surface-sunken border border-border rounded p-3 resize-none focus:outline-none"
                        />
